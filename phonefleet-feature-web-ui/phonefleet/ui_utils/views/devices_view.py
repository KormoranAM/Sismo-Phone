...
def devices_view(fleet: dict[str, Device]):
    ...
    async def stop_and_save(exp_name: str):
        stopped = await run_on_selected_devices(lambda d: d.stop())
        for ip in stopped: grid.run_row_method(ip, "setDataValue", "status", fleet[ip].status.value)

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_DEFLATED) as z:
            for row in await grid.get_selected_rows():
                d = fleet[row["ip"]]
                try:
                    acc = [f for f in await run.io_bound(d.get_file_list) if "accelerometer" in f]
                    if acc:
                        fn = acc[-1]
                        z.writestr(f"{d.name}_{fn}", await run.io_bound(lambda: d.get_file(fn)))
                except Exception as e:
                    logger.error(f"{d.ip}: {e}")

        buf.seek(0)
        ui.download(buf.read(), f"accel_{datetime.now():%Y%m%d_%H%M%S}.zip")
        logger.info(f"Stopped & saved accel for {len(stopped)} device{plural(stopped)}")
    ...
    with ui.row().classes("space-x-4 self-center w-full"):
        ...
        ui.button(
            "Stop and save accelro",
            on_click= stop_and_save,
            color="indigo-400",
            icon="stop",
        )
