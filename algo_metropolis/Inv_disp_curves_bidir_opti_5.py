...
from numba import njit, guvectorize, float64, prange, cuda
import math
import ctypes
is_windows = (os.name == "nt")
if is_windows:
	lib = ctypes.CDLL("./libwavenumbers.dll")  # ou "wavenumbers.dll"
	lib.wavenumbers_stein_squire_c.argtypes = [
	...
	lib.wavenumbers_stein_squire_c.restype = None
else:
	lib = ctypes.CDLL("./libwavenumbers.so")  # ou "wavenumbers.dll"
	lib.wavenumbers_stein_squire_c.argtypes = [
    	ctypes.c_double, ctypes.c_double, ctypes.c_double,
    	ctypes.c_double, ctypes.c_double,
    	np.ctypeslib.ndpointer(dtype=np.float64, ndim=1, flags="C_CONTIGUOUS"),
...
def simulated_annealing(delta_param0, T0, T0param, Tmin, Tminparam, X, MIN_param0, MAX_param0, data, isave):
    ...
    m = freq.size
    kQSsynthetics = np.empty(m, dtype=np.float64)
    
    lib.wavenumbers_stein_squire_c(X[3, 0], X[0, 0], H, X[1, 0], X[2, 0],freq, m,c_w, rho_w, kQSsynthetics)
    ...
    while it <= N_SA - 1: 
        ...
        lib.wavenumbers_stein_squire_c(Xtest[3], Xtest[0], H, Xtest[1], Xtest[2],freq, m,c_w, rho_w, kQSsynthetics)
